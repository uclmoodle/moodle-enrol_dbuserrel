<?php
// This file is part of Moodle - http://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <http://www.gnu.org/licenses/>.

/**
 * Internal (role-based) relationship dataport implementation.
 *
 * @package    enrol_dbuserrel
 * @copyright  2019 Segun Babalola
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

// namespace enrol_dbuserrel\entities;

// use enrol_dbuserrel\interfaces\dataport_interface;

defined('MOODLE_INTERNAL') || die();

Class enrol_dbuserrel_dataport_internal implements enrol_dbuserrel_dataport_interface {

    private $localsubject;
    private $localobject;

    private $localrolefield;

    /**
     * Create new ixternal (i.e. Moodle) dataport object instance.
     *
     * @param array $config
     */
    public function __construct(array $config = []) {

        // Todo: sanitise all input values
        if (isset($config['localsubject'])) {
            $this->set_local_subject(enrol_dbuserrel_field_factory::create(trim(strtolower($config['localsubject']))));

            if (!$this->get_local_subject() instanceof \enrol_dbuserrel_field_interface) {
                throw new \Exception('Attempt to set invalid type for local subject');
            }
        } else {
            throw new \Exception('Local subject is not set in internal data port');
        }

        if (isset($config['localobject'])) {
            $this->set_local_object(enrol_dbuserrel_field_factory::create(trim(strtolower($config['localobject']))));

            if (!$this->get_local_object() instanceof \enrol_dbuserrel_field_interface) {
                throw new \Exception('Attempt to set invalid type for local object');
            }
        } else {
            throw new \Exception('Local object is not set in internal data port');
        }

        if (isset($config['localrole'])) {
            $this->localrolefield = trim(strtolower($config['localrole']));

            if (!is_string($this->localrolefield) || (strlen($this->localrolefield) < 1)) {
                throw new \Exception('Attempt to set invalid local role');
            }
        } else {
            throw new \Exception('Local role is not set in internal data port');
        }
    }

    public function get_all_roles() {
        global $DB;
        return $DB->get_records('role', array(), '', "$this->localrolefield, id", 0, 0);
    }


    public function get_role_fieldname() {
        return $this->localrolefield;
    }

    public function get_subject_fieldname() {
        return $this->get_local_subject()->get_field_name();
    }

    public function get_object_fieldname() {
        return $this->get_local_object()->get_field_name();
    }

    public function get_relationships_in_scope(?string $subjectfilter, ?string $objectfilter)
    {
        global $DB;

        $existingrelationships = array();

        try {
            $uniquekey = $DB->sql_concat("r." . $this->localrolefield, "'|'", "localsubject.id", "'|'", "localobject.id");

            $columns[] = $uniquekey . " AS uniq";
            $columns[] = "ra.roleid";
            $columns[] = "ra.userid";
            $columns[] = "ra.component";
            $columns[] = "ra.contextid";
            $columns[] = "r." . $this->localrolefield;
            $columns[] = "localsubject.id AS subject_id";
            $columns[] = "localobject.id AS object_id";

            $tables[] = " {role_assignments} AS ra ";
            $tables[] = " {role} AS r ";
            $tables[] = " {context} AS c ";
            $tables[] = " {user} AS localsubject ";
            $tables[] = " {user} AS localobject ";

            $sql = "SELECT " .
                implode(",", array_unique($columns)) .
                " FROM " .
                implode(",", array_unique($tables)) .
                " WHERE " .
                " ra.roleid = r.id AND ra.component = 'enrol_dbuserrel' AND c.contextlevel = " . CONTEXT_USER . " AND " .
                " c.id = ra.contextid AND " .
                " ra.userid = localsubject.id AND " .
                " c.instanceid = localobject.id " .
                ($subjectfilter ? " AND localsubject.id=" . $subjectfilter : "") .
                ($objectfilter ? " AND localobject.id=" . $objectfilter : "");

            $existing = $DB->get_records_sql($sql);

            foreach ($existing as $record) {
                /*
                // Construct unique keys
                $subjectsubkey = $this->get_local_subject()->translate_moodle_userid_to_mapped_value($record['subject_id']);
                $objectsubkey = $this->get_local_object()->translate_moodle_userid_to_mapped_value($record['object_id']);

                // This is the unique key in terms of the external fields. This key cannot be used as array key because
                // there is no guarantee each user has a value set, or even if set, that the value hasn't changed.
                $uniquekey = $record[$this->localrolefield] . '|' .
                    (!empty($subjectsubkey) ? $subjectsubkey : '') . '|' .
                    (!empty($objectsubkey) ? $objectsubkey : '');
                */

                // Key the array using unique keys in terms of userid
                 $existingrelationships[$record->uniq] = (array)$record;
            }
        } catch (\Exception $e) {
            throw new \Exception('Unable to query for existing Moodle relationships: ' . $e->getMessage());
        }

        return $existingrelationships;

    }

    private function _get_relationships_in_scope(?string $subjectfilter, ?string $objectfilter) {
        global $DB;

        $existingrelationships = array();

        try {
            $localsubjecttable = $this->localsubject->get_single_table_definition();
            $localobjecttable = $this->localobject->get_single_table_definition();

            // Construct a unique relationship key that allows detection of duplicates
            $uniquekey = $DB->sql_concat(
                "r." . $this->localrolefield, "'|'"
                , $localsubjecttable['table_alias'] . "." . $this->localsubject->get_field_name(), "'|'"
                , $localobjecttable['table_alias'] . "." . $this->localobject->get_field_name()
            );

            $columns = array();
            $tables = array();

            $columns[] = $uniquekey . " AS uniq";
            $columns[] = $localsubjecttable['table_alias'] . ".*";
            $columns[] = $localobjecttable['table_alias'] . ".*";
            $columns[] = "r." . $this->localrolefield;

            $tables[] = " {role_assignments} AS ra ";
            $tables[] = " {role} AS r ";
            $tables[] = " {context} AS c ";
            $tables[] = " {user} AS localsubject ";
            $tables[] = " {user} AS localobject ";
            $tables[] = $localsubjecttable['table_def'] . " AS " . $localsubjecttable['table_alias'];
            $tables[] = $localobjecttable['table_def'] . " AS " . $localobjecttable['table_alias'];

            $sql = "SELECT " .
                implode(",", array_unique($columns)) .
                " FROM " .
                implode(",", array_unique($tables)) .
                " WHERE " .
                " ra.roleid = r.id AND ra.component = 'enrol_dbuserrel' AND c.contextlevel = " . CONTEXT_USER . " AND " .
                " c.id = ra.contextid AND " .
                " ra.userid = localsubject.id AND " .
                " c.instanceid = localobject.id AND " .
                $localsubjecttable['table_alias'] . "." . $this->localsubject->get_userid_join_column() . "=localsubject.id AND " .
                $localobjecttable['table_alias'] . "." . $this->localobject->get_userid_join_column() . "=localobject.id" .
                ($subjectfilter ? " AND localsubject.id=" . $subjectfilter : "") .
                ($objectfilter ? " AND localobject.id=" . $objectfilter : "");

            $existing = $DB->get_records_sql($sql);

            foreach ($existing as $record) {
                if ($record->uniq) {
                    $existingrelationships[$record->uniq] = $existing;
                }
            }
        } catch (\Exception $e) {
            throw new \Exception('Unable to query for existing Moodle relationships: ' . $e->getMessage());
        }

        return $existingrelationships;
    }

    // Todo: improve implementation of this function. It can be more succinct.
    public function get_equivalent_moodle_id($value, $source){
        if ($source == 'subject') {
            return $this->get_local_subject()->get_equivalent_moodle_id($value);
        }  elseif ($source == 'object') {
            return $this->get_local_object()->get_equivalent_moodle_id($value);
        } else {
            return null;
        }

    }

    public function sanitise_literal_for_comparison(string $value) {

    }

    public function construct_unique_relationship_key() {

    }
    private function get_local_object(): enrol_dbuserrel_field_interface {
        return $this->localobject;
    }

    private function set_local_object(enrol_dbuserrel_field_interface $o) {
        $this->localobject = $o;
    }

    private function get_local_subject(): ?enrol_dbuserrel_field_interface {
        return $this->localsubject;
    }

    private function set_local_subject(?enrol_dbuserrel_field_interface $o) {
        $this->localsubject = $o;
    }

}
